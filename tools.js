 ...
 */
const getAverageNoOfRequests = async (timeframe) => {
    ...
    if (txTotalCount) {
        ...
        let avg = (txTotalCount.count / (Date.now() - firstTx.created_at));
 ...
 */
const getNoOfRequests = async (from, to) => {
    const txTotalCount = await db.count('id')
        .from('api_logs')
        .whereBetween('created_at', [from, to]);
    return parseInt(txTotalCount[0].count);
};
/**
